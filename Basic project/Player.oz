...
in
   ...
   fun{FireItem ?KindFire State} % Listfire étrange? version smart buggé donc remplacé par tout con , peut etre trouvée au commit updateplayer du 20/4
	 ...
	 [] missile then
	    KindFire=missile({FindInList State.ide.potPos {Random {List.length State.ide.potPos}}})
   ...
   in
      ...
      %{NewPort Stream Port}
      Stream=[fireItem(?ID2 ?KindFire1) fireItem(?ID2 ?KindFire2) fireItem(?ID2 ?KindFire3)]   
      thread
